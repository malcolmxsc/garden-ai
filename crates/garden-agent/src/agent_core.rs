...
async fn async_main() -> anyhow::Result<()> {
    ...
    match garden_ebpf::tracer::start_tracer(policy).await {
        Ok((handle, mut rx)) => {
            ...
            // to the accept loop.
            //
            // FIX #4: Uses proper async I/O (VsockStream + write_all) instead
            // of raw libc::write. This correctly handles:
            //   - EWOULDBLOCK: waits via epoll instead of dropping the connection
            //   - Partial writes: write_all retries until all bytes are sent
            //   - Backpressure: async write naturally yields to the event loop
            tokio::spawn(async move {
                ...
                let listener_async = match tokio::io::unix::AsyncFd::new(RawFdWrapper(listener_fd)) {
                    Ok(l) => l,
                    Err(e) => {
                ...
                loop {
                    ...
                    let conn_fd = loop {
                        match listener_async.readable().await {
                            ...
                            Err(e) => {
                                tracing::error!("Listener poll error: {}", e);
                    ...
                    // Wrap the accepted fd in VsockStream for proper async I/O
                    let mut stream = match VsockStream::new(conn_fd) {
                        Ok(s) => s,
                        Err(e) => {
                            tracing::error!("Failed to wrap telemetry fd: {}", e);
                    ...
                    loop {
                        match rx.recv().await {
                            Some(event) => {
                                ...
                                // write_all handles partial writes and EWOULDBLOCK
                                if let Err(e) = stream.write_all(json.as_bytes()).await {
                                    tracing::warn!("Telemetry consumer disconnected: {}", e);
                                    break; // VsockStream::drop will close the fd
                                }
                            }
                            None => {
